...
STDMETHODIMP CTVSPropertyArticleDyn::GetDescription (BSTR *pBstrName) {
	if ( pBstrName == NULL )
		return (E_POINTER) ;
	// TODO: add your code here (and comment the line below)
	*pBstrName =::SysAllocString (L"Description for My Property (CTVSPropertyArticleDyn)") ;
...
STDMETHODIMP CTVSPropertyArticleDyn::GetCurrentValueData (IUnknown *pUnk, VARIANT *pVarData) {
	...
	if (TVSController::get()->tvsPropertyController.getSubXString(objId, CTVSProperty, CTVSProperty_Number_article, retValue))
...
STDMETHODIMP CTVSPropertyArticleDyn::SetCurrentValueData (IUnknown *pUnk, const VARIANT varData) {
	...
	AcAxDocLock docLoc(acdbCurDwg());
	TVSController::get()->tvsPropertyController.setSubXString(objId, CTVSProperty, CTVSProperty_Number_article, CString(V_BSTR(&varData)));
